"""Multi-tier allocation with buy cadence filter to reduce entry churn.

Sell immediately on VIX escalation (defensive). Defer buys to once per N days
to avoid micro-reversals when VIX bounces at boundary. Preserves crisis response.
"""
...
class MultiTierAllocatorCadence:
    """Multi-tier allocation with buy cadence filter.

    Tier 1 (VIX ≤ 15): 100% SPY
    Tier 2 (15 < VIX ≤ 20): 100% ISF.L
    Tier 3 (VIX > 20): 100% CSH2.L (defensive/money-market)

    Cadence: Sell defensively immediately. Buy/upgrade only if >N days since last buy.
    Prevents daily flips when VIX bounces at boundary.
    """
    ...
    def __init__(
        ...
        buy_cadence_days: int = 5,
    ):
        """Initialize multi-tier allocator with buy cadence.
        ...
        Args:
            ...
            buy_cadence_days: Only allow buy/upgrade if >N days since last buy
        """
        self.vix_tier1 = vix_tier1
        self.vix_tier2 = vix_tier2
        self.buy_cadence_days = buy_cadence_days

    def _compute_raw_tier(self, vix: float | None) -> int:
        """Compute tier based on VIX level alone."""
        if vix is None:
            return 1
    ...
    def _is_defensive_move(self, current_tier: int, desired_tier: int) -> bool:
        """Check if move is defensive (escalate to lower-tier asset for crisis)."""
        return desired_tier > current_tier

    def _is_offensive_move(self, current_tier: int, desired_tier: int) -> bool:
        """Check if move is offensive (de-escalate to riskier asset for return)."""
        return desired_tier < current_tier
    ...
    def backtest(
        ...
        """Run multi-tier allocation backtest with buy cadence buffering.
        ...
        # Track current tier and days since last buy (offensive move)
        ...
        for i, date in enumerate(dates):
            ...
            # Apply cadence: allow defensive moves immediately, defer offensive moves
            if self._is_defensive_move(current_tier, desired_tier):
                # Escalate to safety immediately
                current_tier = desired_tier
                days_since_last_buy = 0
            elif self._is_offensive_move(current_tier, desired_tier):
                # Only de-escalate (buy/upgrade) if enough days have passed
                if days_since_last_buy >= self.buy_cadence_days:
                    current_tier = desired_tier
                    days_since_last_buy = 0
                else:
            ...
            # Override signal tier with cadence-filtered tier
            ...
            signal = MultiTierSignal(
                ...
                tier=current_tier,
                asset=current_asset,
                reason=signal.reason + f" [cadence: tier {current_tier}]",
            )
