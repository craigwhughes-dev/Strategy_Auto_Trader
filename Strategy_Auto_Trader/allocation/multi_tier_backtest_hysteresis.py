"""Hysteresis sweep for multi-tier allocation: test buffering days 1-10.

Goal: Find optimal hysteresis_days to minimize whipsaw cost in choppy years
(2005, 2015) without sacrificing returns in trending years (2012, 2019, etc).
"""
...
from .multi_tier_allocator_hysteresis import MultiTierAllocatorHysteresis
...
def main():
    parser = argparse.ArgumentParser(description="Multi-tier allocation hysteresis sweep")
    ...
    parser.add_argument("--hysteresis-days", type=int, nargs="+", default=[1, 2, 3, 5, 10], help="Days to buffer")
    ...
    print("\n" + "=" * 120)
    print("MULTI-TIER ALLOCATION: HYSTERESIS SWEEP")
    print("=" * 120)
    ...
    print("| Hysteresis | Sharpe | Sortino | Return % | Max DD % | Final Value | Switches | Bad Years (2005/2015) |")
    print("|---|---|---|---|---|---|---|---|---|")

    for hyst_days in sorted(args.hysteresis_days):
        allocator = MultiTierAllocatorHysteresis(
        ...
        print(f"|{hyst_days:11d} | {summary['sharpe']:6.2f} | {summary['sortino']:7.2f} | {summary['total_return_pct']:9.2f} | {summary['max_drawdown_pct']:7.2f} | ${final_value:13,.0f} | {int(total_switches):8d} | {bad_years_str:>20s} |")
