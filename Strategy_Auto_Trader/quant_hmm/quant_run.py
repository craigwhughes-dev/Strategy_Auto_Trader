...
def _print_results(bt: dict, args, detail: pd.DataFrame) -> None:
    ...
    logger.info(f"  {'Information ratio':30s}  {_f(bt.get('information_ratio', float('nan'))):>10s}")
    logger.info(f"  {'Up / down capture':30s}  {_f(bt.get('up_capture', float('nan'))):>10s}  {_f(bt.get('down_capture', float('nan'))):>10s}")
    ...
    if n_trades:
        ...
        logger.info(f"  Kelly fraction: {bt['final_kelly']*100:.1f}%")
    else:
        logger.info(f"\n  No completed trades.")
    ...
    logger.info(f"  {'Strategy':20s}  {sign}GBP{abs(pl):,.2f}  ({sign}{pl/ic*100:.1f}%)")
    logger.info(f"  {'Buy & Hold':20s}  {bh_sign}GBP{abs(bh_pl):,.2f}  ({bh_sign}{bh_pl/ic*100:.1f}%)")

    last = detail.iloc[-1]
    logger.info(f"\n  Current state:")
    logger.info(f"  P(Bull): {last['p_bull']:.1%}  |  P(Bear): {last['p_bear']:.1%}")
    logger.info(f"  Volume ratio: {last['volume_ratio']:.1f}x avg")
    in_pos = last["position"] > 0
    if in_pos:
        logger.info(f"  IN POSITION  entry={last['entry_price']:.2f}  "
              f"stop={last['stop_level']:.2f}  target={last['target_level']:.2f}")
    else:
        logger.info(f"  FLAT — waiting for P(Bull) > {args.entry_prob:.0%} + volume confirmation")
...
def _print_exit_breakdown(detail: pd.DataFrame, n_trades: int) -> None:
    ...
    for reason_prefix in ["stop_loss", "take_profit", "regime_exit"]:
        ...
        if count:
            ...
            logger.info(f"    {reason_prefix:<15s}: {count:>3d} trades, avg P&L {avg_pl:+.1f}%")


def _print_sentiment_detail(sent_data: dict) -> None:
    logger.info(f"\n  Sentiment & Alternative Data:")
    logger.info(f"  {'Composite':20s}  {sent_data['sentiment_label'].upper():>10s}  "
    ...
    if opts.get("put_call_ratio") is not None:
        ...
        logger.info(f"  {'Put/Call ratio':20s}  {opts['put_call_ratio']:>10.3f}  "
    ...
    if si.get("short_pct_float") is not None:
        logger.info(f"  {'Short % of float':20s}  {si['short_pct_float']:>9.2f}%  "
              f"days-to-cover={si.get('short_ratio', 'N/A')}")


def main(argv: list[str] | None = None) -> int:
    setup_cli_logger("quant_run")

    args = _build_arg_parser().parse_args(argv)

    logger.info(f"\nQuant HMM Engine — {args.ticker}")
    logger.info(f"  Hourly data, HMM regime probabilities, volume confirmation, Kelly sizing")
    logger.info(f"  Entry: P(Bull)>{args.entry_prob}  Exit: P(Bull)<{args.exit_prob}")
    logger.info(f"  Stop: {args.stop_loss*100:.0f}%  Target: {args.take_profit*100:.0f}%  Kelly: {'on' if args.kelly else 'off'}")

    logger.info(f"\n  Fetching hourly data for {args.ticker}...")
    t0 = time.time()
    df = fetch_hourly(args.ticker, period=args.period)
    if df is None or df.empty:
        logger.info(f"  No data for {args.ticker}")
        return 1
    ...
    try:
        ...
        logger.info(f"  {name}  [{sector}]" if sector else f"  {name}")
    except Exception:
        name = args.ticker
        sector = ""
    ...
    if args.sentiment:
        ...
        logger.info(f"  Sentiment: {sent_data['sentiment_label']} ({sent_score:+.3f})  "
    ...
    if detail.empty:
        logger.info("  Insufficient data for backtest.")
        return 1
