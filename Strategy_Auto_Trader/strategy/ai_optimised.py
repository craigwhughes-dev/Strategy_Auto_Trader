...
------------------------------------
AI (ensemble heuristic) strategy wins on stocks with strong bull regimes.
Analysis shows AI winners (r=0.604) greatly outperform losers (r=0.348).
This variant adds a regime-strength gate: only enter if the HMM model
detects a strong bull probability (P_Bull > 0.6). This eliminates entries
in weak/choppy markets where the ensemble score triggers despite lacking
regime confirmation.

Best suited to: same as AI but with regime confirmation. Fewer trades but
higher win rate on the entries that pass.

Entry
-----
Same as AI (tanh-normalized ensemble: regime 0.45 + RSI 0.35 + displacement
0.15 + volume 0.05, threshold 0.4) PLUS additional regime gate: **P(Bull) > 0.6**
at entry.

Exit
----
Identical to AI: 5% stop-loss, 18% take-profit. Kelly position sizing.
"""
...
class AiOptimisedEntry:
    """Heuristic ensemble entry with strong bull regime gate.
    ...
    def evaluate(
        ...
        Filters out entries in weak/choppy markets despite ensemble score.
        """
