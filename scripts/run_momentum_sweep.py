#!/usr/bin/env python3
"""
momentum_weight x momentum_lookback_days sweep.

Adds JT price momentum factor to top-K ticker scoring. Candidates generated
ONCE per window; only filter_candidates_by_top_tickers() and arbitrate() re-run
per combo.

Windows:
  - 2023 window (synthetic 2022-01-01 to 2024-12-31): tests whether momentum
    factor captures AI names (NVDA +239%, META +91%) that drove the 2023 rally.
  - Real window (ibkr, 2024-01-01 to present): checks production impact.

Usage:
    uv run python scripts/run_momentum_sweep.py
"""
...
# momentum_weight x momentum_lookback_days combos
# 0.0 = baseline (TQ + win-rate only)
MOMENTUM_WEIGHTS = [0.0, 0.1, 0.2, 0.3]
MOMENTUM_LOOKBACKS = [126, 252]   # 6mo, 12mo
...
def _run_combo(label, candidates_all, trend_quality_by_ticker, price_by_ticker,
               vix_series, vix_threshold, momentum_weight, momentum_lookback_days):
    logger = logging.getLogger(__name__)
...
def main():
    setup_cli_logger("momentum_sweep")
    logger = logging.getLogger(__name__)
    ...
    for mw in MOMENTUM_WEIGHTS:
        lookbacks = MOMENTUM_LOOKBACKS if mw > 0.0 else [252]
        for ml in lookbacks:
            tag = f"mw{int(mw * 10)}_ml{ml}"
            label = f"momentum_{tag}_2023"
            logger.info(f"\n  --- momentum_weight={mw} lookback={ml} (2023 window) ---")
            r = _run_combo(label, synth_cands_raw, synth_tq, synth_prices,
                           synth_vix, vix_threshold, mw, ml)
            r["window"] = "2023"
            results.append(r)
    ...
    for mw in MOMENTUM_WEIGHTS:
        ...
        for ml in lookbacks:
            tag = f"mw{int(mw * 10)}_ml{ml}"
            label = f"momentum_{tag}_real"
            logger.info(f"\n  --- momentum_weight={mw} lookback={ml} (real window) ---")
            r = _run_combo(label, real_cands_raw, real_tq, real_prices,
                           real_vix, vix_threshold, mw, ml)
            r["window"] = "real"
            results.append(r)
    ...
    #  Summary                                                             #
    # ------------------------------------------------------------------ #
    logger.info("\n\n=== MOMENTUM SWEEP SUMMARY ===")
    logger.info(f"{'mom_weight':>10} | {'mom_lookback':>12} | {'window':>8} | {'return%':>8} | {'max_dd':>7} | {'admitted':>8} | top-10 (sample)")
    logger.info("-" * 110)
    for r in results:
        top10 = ", ".join(r["top_tickers"][:10])
        logger.info(f"{r['momentum_weight']:>10.1f} | {r['momentum_lookback_days']:>12} | "
                    f"{r['window']:>8} | {r['return_pct']:>8.1f} | "
    ...
    summary_path = JOURNAL_DIR / "momentum_sweep_summary.csv"
